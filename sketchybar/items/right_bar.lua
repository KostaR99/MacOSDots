...
local cpu_temp = require("items.widgets.cpu_temp")
...
local systray   =
    sbar.add(
        "bracket",
        "systray.bracket",
        { wifi.name, media.name, weather.name, cpu_temp.name },
        {
            display = 1,
            width = "dynamic",
            icon = {
...
local right_bar =
    sbar.add(
        "bracket",
        "right_bar.bracket",
        { clock.name, volume_outer.name, systray.name },
        {
